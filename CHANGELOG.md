...
### Added
- **Concurrent triage**: `autofte triage` / `autofte pipeline` now classify
  and reproduction-verify crash files in a thread pool instead of one at a
  time (`--workers N`, default `min(8, cpu count)`). Aggregation (groups,
  counts, `seen`) stays fully deterministic regardless of worker count --
  same output as serial triage, just faster. ~4.8x wall-clock speedup
  measured on a 12-crash synthetic corpus with default workers.
