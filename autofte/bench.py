...
the project methodology: it never re-fuzzes or re-runs a target.
...
# Analysis 1 (the project methodology E2/E3 aggregation question, resolved
# 2026-08-08, see benchmarks/results.md "Analysis 1 -- aggregation methodology
...
# Confirmed independently by a independent review; the one open
...
# spec bug, fixed in the project methodology alongside this change) --
...
AGGREGATION_NOTE = (
    ...
    "not quoted -- see benchmarks/results.md 'Analysis 1' for the full citation "
...
def _macro_metrics(per_target):
    ...
    AGGREGATION_NOTE and benchmarks/results.md 'Analysis 1'). Each target counts
...
def render_aggregation_table(result):
    ...
    axis to average over. See the project methodology E2/E3 and
    benchmarks/results.md 'Analysis 1'/'Analysis 2'.
    """
    metrics = result.get("metrics")
