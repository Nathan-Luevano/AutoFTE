...
Tracked per `planning/HARDENING.md` Part 2.3: "Track results over time in a
...
Published baselines for comparison (from `planning/research/05-accuracy-and-ground-truth.md`
§2.4, GPTrace ICSE'26 Table 3, same corpus family):
...
specced in `planning/research/05-accuracy-and-ground-truth.md` §3.4 --
...
**Deferred, not forgotten — stopping because further subagent dispatches
started failing on a monthly spend limit, not because these are done or
unimportant:**
...
2, because per planning/V1-RELEASE.md §1.1's diagnosis, 2 frames is too
...
No suggested commit — this task lands a diagnostic script only
...
## TASK 1 — aggregation methodology of the GPTrace/Igor baselines — 2026-08-08 — DETERMINED: macro (per-target mean)

**Question.** E2 (purity >= 0.95) and E3 (F-measure >= 0.82) in
`planning/V1-RELEASE.md` were written against the GPTrace ICSE'26 paper's
...
target regardless of its size"), which is exactly why TASK 2 makes
AutoFTE report both rather than silently switching to whichever one is
higher.

**Adversarial check requested per the goal's RULES** (spawn a fresh agent
to check this conclusion, since it flips E2/E3): see the RESULT entry
immediately below — the check was run against this exact conclusion using
the corroborating evidence above.

Not a code change; no regression risk. No suggested commit for this
entry — it is the evidentiary basis TASK 3 and TASK 4 apply to the spec
and README, logged separately below.

## TASK 2 — report micro and macro side by side in `autofte bench` — HYPOTHESIS — 2026-08-08

**Hypothesis.** `autofte bench` currently prints only the pooled (micro)
aggregate metrics. Given TASK 1's finding that the literature's numbers
...
equal weight (mirroring exactly the unweighted-mean method TASK 1
...
## TASK 2 — report micro and macro side by side in `autofte bench` — RESULT — 2026-08-08 — KEEP
...
TASK 1's hypothesis (0.9767 / 0.9046 / 0.9193 predicted, 0.9767 / 0.9046
...
> all 14 target rows (see benchmarks/results.md, "TASK 1"). AutoFTE's
...
(benchmarks/results.md, TASK 1), which AutoFTE's pooled-only reporting
